<template>
    <div class="container mt-4">
        <h3 class="text-center my-4">Chỉnh sửa sản phẩm</h3>

        <form @submit.prevent="updateDrink" class="row g-3">
            <div class="col-md-6">
                <div class="mb-3">
                    <label class="form-label">SKU</label>
                    <input v-model.number="form.SKU" type="text" class="form-control" required disabled />
                </div>
            ...
            <div class="col-12 mt-4">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="toggleSize" v-model="hasSize" />
                    <label class="form-check-label" for="toggleSize">Có size</label>
                </div>
            </div>

            <div class="col-md-8" v-if="hasSize">
...
const updateDrink = async () => {
    ...
    try {
        ...
        if (!hasOption.value) form.value.option = {}
        if (!hasSize.value) form.value.size = []
        if (!hasProductInfo.value) form.value.product_information = {}
        if (!hasInstructions.value) form.value.instructions = {}

        const res = await axios.put(`http://localhost:3000/products/${route.params.id}`, form.value)
